...
describe("migration-safety", () => {
  ...
  test.each([
    ...
    'ALTER TABLE "u" ADD COLUMN "c" serial NOT NULL;',
    'ALTER TABLE "m" ALTER COLUMN "p" ADD GENERATED BY DEFAULT AS IDENTITY;',
  ...
  test.each([
    ...
    [
      'ALTER TABLE "m" ALTER COLUMN "p" ADD GENERATED ALWAYS AS IDENTITY;',
      "GENERATED ALWAYS への変更",
    ],
  ...
  test.each([
    ...
    "COMMENT ON TABLE t IS 'x';",
    "CREATE FUNCTION f() RETURNS int LANGUAGE plpgsql AS 'BEGIN ALTER TABLE users DROP COLUMN email; RETURN 1; END';",
    "CREATE RULE r AS ON INSERT TO users DO INSTEAD NOTHING;",
    'ALTER SEQUENCE "s" INCREMENT BY 3;',
  ])("読めない書き方は止める: %s", (sql) => {
    expect(findViolations(sql).map((v) => v.reason)).toContain(
      "このチェックが読まない書き方（手書きの SQL か、ここで扱わない種類の変更）",
    );
  });
