...
Observability データセットに入る。そのためこの規約は両方に同じく適用される。

- **生の `console.*` は使わない**。
  - api-service: `c.get("logger")` / DI された `logger`
  - client のサーバー側: `shared/lib/server-logger.ts` の `serverLogger`
  - **UI コンポーネントも対象**。SSR 時はサーバーで描画されるため、描画時の `console` は
    そのまま Workers ログに出る（「ブラウザで動くから対象外」は成り立たない）
- **third-party のロガーも同じ出力先に寄せる**。console 出力するライブラリを入れるときは、
  ...
  - `auth.api.getSession` を直接呼ぶ箇所（`features/auth/infrastructure/session.ts`）: Better Auth は DB 障害を
...
- **`error` / `err` キーは「5xx・未捕捉例外」専用**。Cloudflare はこの2つのキーの値を
  ...
  本物の異常が埋もれる（計測したところ、1 日の Errors の大半が正常な 401 だった）。4xx の理由は `errorCode`、その他は `reason` / `detail` 等でよい。
...
## なぜ生の console を禁止するか（計測メモ）

使い捨て Worker を本番アカウントへ一時デプロイし、3レベル×14形状で計測した結果、
...
生の console を残すとこの2系統が混在し、規約を二重に書く羽目になる。出力先を1つにすれば
