...
後から一切計測できなくなる。N は `/review-full`（または `/code-review`）を回した周の数で、指摘ゼロで終わった
最後の周も含める（1 周目で指摘ゼロなら `レビュー往復: 1周（主な指摘: なし）`）。指摘が無くても書く。

**計測用の記録（任意。書けるときは書く）** — `bun run metrics`（`scripts/dev/dev-metrics.mjs`）が集計する:
- `レビュー検出: ①N ②N ③N` — `/review-full` の全周で、各レビュアーが出した CONFIRMED（🔴🟠）の件数。
  同じ指摘を複数が出したら全員に数える。③をスキップした周しか無ければ `③-`。③（Codex）が①②に
  上乗せしているかを数字で判断するため
- `原因PR: #N` — 不具合修正の PR で、その不具合を入れた PR が分かるとき（複数なら `#N, #M`）。
  レビューを通ったのにマージ後に見つかった不具合（取りこぼし）を数えるため。どの周回数・どの種別の
  PR から漏れているかが分かる

### 5. レビュー収束とマージ

PR を作ったら、**worktree の有無に関係なく PR 番号を渡して** `/review-full <PR番号>` を回す
（実装文脈を持たないサブエージェント2体と、別系統のモデル（Codex CLI。未導入ならスキップ）による独立レビュー。修正と修正差分限定の再レビューを
**収束まで内包する**ので外側で周回しない）。小さな PR（設定・文言だけ等）で重すぎるなら
`/code-review medium <PR番号>` でもよく、その場合は CONFIRMED の指摘が出たら修正して push し、
もう 1 周。**CONFIRMED がゼロになった周で終了**（PLAUSIBLE や整理系だけの周は打ち切ってよい）。
依存更新だけの PR でも省かない。
