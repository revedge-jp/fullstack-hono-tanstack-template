...
ローカルの `main` は `origin/main` より古いことがあり、そのまま比べると無関係な差分が含まれる
...
- `[考慮漏れ候補]` — 削除・変更によって何かに不具合が起きる可能性がある
...
## Step 3: プロジェクト固有の注意点チェック（Step 2 の後に実行）
...
- `andThen` / `asyncAndThen` / `map` の使い分けが正しいか。`ResultAsync` の `andThen` は `Result` と `ResultAsync` の
  どちらを返す関数も受けるので、`okAsync(input).andThen(step)`（参照実装と同じ形）は正しい。見るのは次の 3 つ:
...
- 一緒に成功・失敗すべき複数の書き込みが、リポジトリ（infrastructure）の中で `db.transaction(async (tx) => ...)` に囲まれているか（usecase は DB を直接触らないので、トランザクションもリポジトリのメソッド1つにまとめる）
...
- 参照実装（`apps/api-service/src/features/tasks`・`apps/api-service/src/integrations/composition`・
  ...
  - 実害: サンプルは新しい feature を書くときの型紙として写されるので、ずれた形がそのまま量産される
    （文字列の一致では確認できないため機械的なチェックにはしていない）
...
- 🔴 確実に不具合が起きる / セキュリティホール
- 🟠 特定条件下で不具合が起きる / エラーが握りつぶされる / 新機能の異常系が未実装
- 🟡 ルール違反だが即時障害にはならない
