...
| `PLANETSCALE_SERVICE_TOKEN_ID` | PlanetScale DB / Role の作成。**Environment ごとに別に発行**し、production 用は production にだけ置く（発行時の権限のままだと組織の全 DB を削除できるため。[Alchemy IaC ガイド](../dev/alchemy-iac.md#state-と資格情報の権限境界preview-を使う前に読む)）。発行時のトークン名は `<APP_NAME>-<stage>` 推奨 |
