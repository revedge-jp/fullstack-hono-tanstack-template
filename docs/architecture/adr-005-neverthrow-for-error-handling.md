...
- **握り忘れがコンパイルを通る**: `try/catch` を書き忘れても、対応していない分岐があっても、
  ビルドは成功する。問題が表面化するのは本番でそのエラーが実際に起きた時点（ランタイム）。
- **エラー種別の網羅性を保証できない**: 新しいエラー種別を追加しても、
  対応していない呼び出し元が機械的には洗い出されない。
...
|---|---|---|
| タイミング | コードを動かす**前** | 動いて失敗した**後** |
| 目的 | バグの**予防**（障害を起こさない） | バグの**診断**（障害を調べる） |

握り忘れを防げているほど、ログに残る異常は「想定済みの種別」と
