...
| Integration（実DB） | Drizzle クエリ・DB制約の実挙動 | `bun run test:integration` | ✓（api-service の `test` が `*.int.test.ts` も含めて走らせる。DB 必須） | ✓（別ジョブ） |
...
| 指示ファイル参照整合 | AGENTS.md / `.claude/rules` 等のパス・`bun run`・見出し参照の実在 | `bun run check:instructions` | ✗ | ✓（`instructions` ジョブ。`ci` ジョブの arch:check にも含まれる） |
...
| 依存脆弱性（bun audit） | allowlist（`.github/security-audit-allowlist.json`）に無い既知の advisory | `bun run check:security-audit` | ✗ | 週次のみ（`security-audit.yml`。PR では走らず、該当があれば issue を立てる） |
...
- **pre-push（`bun run check-all`）= 速い中核**。lint/type/test/arch/guards を回す。lint・typecheck・テストは
  ...
  api-service が対象に入るとその `test` が integration も走らせるので、DB が無いと push できない。
...
- [テストガイド](./testing.md)（書くテストの正典への案内と、書き始めに開く実物）
- [機能追加の入口](./adding-features.md)（feature を足すときに読む正典と参照実装）
